  ...
  return (
    <DashboardLayout pageTitle="Banques & Créances">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-[12px] mb-[24px]">
        {[
          { label: 'Total facturé',   value: `${formatAmount(totals.total)} FCFA`,        color: 'text-[#1A1A19]' },
          { label: 'Total encaissé',  value: `${formatAmount(totals.totalPaye)} FCFA`,     color: 'text-[#2D6A4F]' },
          { label: 'En attente',      value: `${formatAmount(totals.totalEnCours + totals.totalNonSolde)} FCFA`, color: 'text-[#8B4513]' },
          { label: 'Échéances dépassées', value: totals.enRetard,                          color: totals.enRetard > 0 ? 'text-[#9B2335]' : 'text-[#1A1A19]' },
        ].map(k => (
      ...
      {enRetardList.length > 0 && (
        <div className="bg-[#FCEBEB] border border-[#9B2335] rounded-[8px] p-[12px_16px] mb-[20px]">
          <div className="flex items-center gap-[8px] mb-[8px]">
            ...
            <span className="text-[13px] font-medium text-[#9B2335]">
          ...
          <div className="flex flex-col gap-[4px]">
            {enRetardList.map(inv => (
              <div key={inv.id} className="flex flex-wrap items-center justify-between gap-[8px]">
                <span className="text-[12px] text-[#9B2335]">
                ...
                <button onClick={() => handleReminder(inv.id, inv.ref)} disabled={isPending}
                  className="text-[11px] text-[#9B2335] border border-[#9B2335] bg-[#FCEBEB] py-[2px] px-[8px] rounded-[4px] hover:bg-[#f7d5d8] disabled:opacity-50">
                  Relancer
                </button>
              </div>
            ))}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-[12px] mb-[20px]">
        ...
        <div className="flex gap-[8px]">
          <Link href="/dashboard/dettes"
            className="border border-[#E8E7E4] bg-[#FFFFFF] text-[#1A1A19] py-[8px] px-[14px] rounded-[6px] text-[13px] font-medium hover:bg-[#F7F7F6] transition-colors">
            Dettes internes
          </Link>
      ...
      <div className="bg-[#FFFFFF] border border-[#E8E7E4] rounded-[10px] overflow-hidden">
        {loading ? (
          ...
          <div className="p-[48px] text-center text-[13px] text-[#6B6A67]">
          ...
          <div className="w-full overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-[#F7F7F6] border-b border-[#E8E7E4]">
                <tr>
                  <th className="py-[12px] px-[16px] text-[12px] font-medium text-[#6B6A67] whitespace-nowrap">Référence</th>
                  <th className="py-[12px] px-[16px] text-[12px] font-medium text-[#6B6A67] whitespace-nowrap">Partenaire</th>
                  <th className="py-[12px] px-[16px] text-[12px] font-medium text-[#6B6A67] whitespace-nowrap">Description</th>
                  <th className="py-[12px] px-[16px] text-[12px] font-medium text-[#6B6A67] whitespace-nowrap text-right">Montant</th>
                  <th className="py-[12px] px-[16px] text-[12px] font-medium text-[#6B6A67] whitespace-nowrap">Dates</th>
                  <th className="py-[12px] px-[16px] text-[12px] font-medium text-[#6B6A67] whitespace-nowrap">Statut</th>
              ...
              <tbody>
                {invoices.map(inv => {
                  ...
                  return (
                    <tr key={inv.id} className={`border-b border-[#E8E7E4] hover:bg-[#F7F7F6] ${late ? 'bg-[rgba(155,35,53,0.02)]' : ''}`}>
                      <td className="py-[13px] px-[16px]">
                        ...
                        {inv.fileUrl && (
                          <a href={inv.fileUrl} target="_blank" rel="noreferrer"
                            className="block text-[11px] text-[#1A3A5C] hover:underline mt-[1px]">PDF</a>
                        )}
                      </td>
                      <td className="py-[13px] px-[16px]">
                        <Link href={`/dashboard/partners/${inv.partner.id}`}
                          className="text-[13px] text-[#1A1A19] hover:text-[#1A3A5C] hover:underline">
                          {inv.partner.orgName}
                        </Link>
                      </td>
                      <td className="py-[13px] px-[16px] text-[12px] text-[#6B6A67] max-w-[180px]">
                        <span className="truncate block">{inv.description}</span>
                        {inv.notes && <span className="block text-[11px] text-[#8B4513] italic mt-[2px] truncate">{inv.notes}</span>}
                      </td>
                      <td className="py-[13px] px-[16px] text-right">
                        <span className="text-[13px] font-medium text-[#1A1A19]">{formatAmount(inv.amount)}</span>
                        <span className="text-[11px] text-[#6B6A67] ml-[3px]">FCFA</span>
                      </td>
                      <td className="py-[13px] px-[16px]">
                        <div className="text-[12px] text-[#6B6A67]">Émis : {formatDateShort(inv.issueDate)}</div>
                        {inv.dueDate && (
                          <div className={`text-[12px] mt-[1px] ${late ? 'text-[#9B2335] font-medium' : 'text-[#6B6A67]'}`}>
                            Éch. : {formatDateShort(inv.dueDate)} {late ? '⚠' : ''}
                          </div>
                        )}
                        {inv.reminderSentAt && (
                          <div className="text-[11px] text-[#6B6A67] mt-[1px] italic">
              ...
              <tfoot className="bg-[#F7F7F6]">
                <tr>
                  <td colSpan={3} className="py-[12px] px-[16px] text-[12px] font-medium text-[#1A1A19]">
                    Total ({invoices.length} facture{invoices.length > 1 ? 's' : ''})
                  </td>
                  <td className="py-[12px] px-[16px] text-right">
                    <span className="text-[13px] font-medium text-[#1A1A19]">
