...
const STEP_CFG: Record<string, { label: string; cls: string; bar: string }> = {
  A_VENIR:  { label: 'À venir',    cls: 'border-[var(--border)] text-[var(--text-secondary)] bg-[var(--bg-surface)]', bar: 'bg-[var(--border)]' },
  EN_COURS: { label: 'En cours',   cls: 'border-[var(--msp-amber)] text-[var(--msp-amber)] bg-[var(--msp-amber-light)]', bar: 'bg-[var(--msp-amber)]' },
  TERMINE:  { label: 'Terminé',    cls: 'border-[var(--msp-green)] text-[var(--msp-green)] bg-[var(--msp-green-light)]', bar: 'bg-[var(--msp-green)]' },
  RETARD:   { label: 'En retard',  cls: 'border-[var(--msp-red)] text-[var(--msp-red)] bg-[var(--msp-red-light)]', bar: 'bg-[var(--msp-red)]' },
  ANNULE:   { label: 'Annulé',     cls: 'border-[var(--border)] text-[var(--text-secondary)] bg-[var(--bg-surface)]', bar: 'bg-[var(--border)]' },
};
const CONT_CFG: Record<string, string> = {
  EN_COURS:  'border-[var(--msp-red)] text-[var(--msp-red)] bg-[var(--msp-red-light)]',
...
const ic = "w-full p-[9px_12px] border border-[var(--border)] rounded-[8px] text-[13px] bg-[var(--bg-card)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] transition-colors";
const lc = "block text-[11px] font-medium text-[var(--text-primary)] mb-[4px]";
...
export default function MarcheDetailPage({ params }: { params: { id: string } }) {
  ...
  if (!market) return (
    <DashboardLayout pageTitle="Introuvable">
      <div className="p-[48px] text-center text-[13px] text-[var(--msp-red)]">Marché introuvable.</div>
    </DashboardLayout>
  );
    ...
    <DashboardLayout pageTitle={market.name}>
      ...
      {feedback.msg && (
        <div className={`mb-[16px] rounded-[8px] p-[10px_14px] text-[13px] border ${
          feedback.ok ? 'bg-[var(--msp-green-light)] border-[var(--msp-green)] text-[var(--msp-green)]' : 'bg-[var(--msp-red-light)] border-[var(--msp-red)] text-[var(--msp-red)]'
        }`}>{feedback.msg}</div>
      )}
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-[20px]">
          ...
          <div>
            <div className="flex items-center gap-[10px] flex-wrap mb-[6px]">
              <h2 className="text-[18px] font-medium text-[var(--text-primary)]">{market.name}</h2>
              <span className={`inline-block border rounded-[6px] py-[2px] px-[8px] text-[12px] ${cfg.cls}`}>{cfg.label}</span>
            </div>
            ...
            {market.description && (
              <p className="text-[13px] text-[var(--text-secondary)] mt-[6px] leading-[1.6]">{market.description}</p>
            )}
            <div className="flex flex-wrap gap-[16px] mt-[12px] text-[12px] text-[var(--text-secondary)]">
              {market.startDate && <span>Début : <strong className="text-[var(--text-primary)]">{formatDateShort(market.startDate)}</strong></span>}
              {market.endDate   && <span>Fin prévisionnelle : <strong className="text-[var(--text-primary)]">{formatDateShort(market.endDate)}</strong></span>}
              {market.nextReviewDate && <span>Prochaine revue : <strong className="text-[var(--msp-amber)]">{formatDateShort(market.nextReviewDate)}</strong></span>}
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-[12px]">
            <div className="text-right">
              <div className="text-[28px] font-medium text-[var(--accent-primary)]">{market.executionRate}%</div>
              <div className="text-[12px] text-[var(--text-secondary)]">Taux d&apos;exécution</div>
              <div className="w-[120px] h-[6px] bg-[var(--border)] rounded-full overflow-hidden mt-[6px]">
                <div className="h-full bg-[var(--accent-primary)] transition-all" style={{ width: `${market.executionRate}%` }}></div>
              </div>
            </div>
          ...
          return (
            <button key={t} onClick={() => setTab(t)}
              className={`py-[10px] px-[16px] text-[13px] border-b-2 transition-colors whitespace-nowrap ${
                tab === t ? 'text-[var(--text-primary)] font-medium border-[var(--accent-primary)]' : 'text-[var(--text-secondary)] border-transparent hover:text-[var(--text-primary)]'
              }`}>
              {t} {count > 0 && <span className="ml-[4px] text-[11px] text-[var(--text-secondary)]">({count})</span>}
            </button>
          );
        })}
      </div>
        ...
        <div>
          <div className="flex justify-between items-center mb-[20px]">
            ...
            <button onClick={() => { setShowStepForm(true); setEditStep(null); }}
              className="bg-[var(--accent-primary)] text-[var(--bg-card)] py-[8px] px-[14px] rounded-[8px] text-[12px] font-medium hover:bg-[var(--msp-blue-mid)] transition-colors">
              + Ajouter une étape
            </button>
          </div>
          ...
          {(showStepForm || editStep) && (
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[10px] p-[20px] mb-[20px]">
              <h3 className="text-[14px] font-medium text-[var(--text-primary)] mb-[16px]">
              ...
              <form onSubmit={handleStepSubmit}>
                ...
                <div className="flex gap-[10px] justify-end">
                  <button type="button"
                    ...
                    className="border border-[var(--border)] text-[var(--text-primary)] py-[8px] px-[14px] rounded-[8px] text-[12px] font-medium hover:bg-[var(--bg-surface)]">
                    Annuler
                  </button>
                  <button type="submit" disabled={isPending}
                    className="bg-[var(--accent-primary)] text-[var(--bg-card)] py-[8px] px-[14px] rounded-[8px] text-[12px] font-medium hover:bg-[var(--msp-blue-mid)] disabled:opacity-60">
          ...
          {market.steps.length === 0 ? (
            ...
            <div className="flex flex-col gap-[10px]">
              {market.steps.map((step, idx) => {
                ...
                return (
                  <div key={step.id}
                    className={`bg-[var(--bg-card)] border rounded-[10px] p-[16px_20px] ${
                      isLate ? 'border-[var(--msp-red)]' : 'border-[var(--border)]'
                    }`}>
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-[10px]">
                      ...
                      <div className="flex items-start gap-[12px] flex-1">
                        ...
                        <div className={`w-[28px] h-[28px] rounded-full flex items-center justify-center text-[11px] font-medium flex-shrink-0 ${
                          step.status === 'TERMINE' ? 'bg-[var(--msp-green-light)] text-[var(--msp-green)]'
                          : step.status === 'EN_COURS' ? 'bg-[var(--msp-amber-light)] text-[var(--msp-amber)]'
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-[8px] flex-wrap">
                            ...
                            {isLate && (
                              <span className="inline-block border border-[var(--msp-red)] text-[var(--msp-red)] bg-[var(--msp-red-light)] rounded-[6px] py-[1px] px-[6px] text-[11px]">
                                ⚠ En retard
                              </span>
                            )}
                          </div>
                          {step.description && (
                            <p className="text-[12px] text-[var(--text-secondary)] mt-[3px]">{step.description}</p>
                          )}
                          <div className="flex flex-wrap gap-[12px] mt-[6px] text-[12px] text-[var(--text-secondary)]">
                            ...
                            {days !== null && step.status !== 'TERMINE' && step.status !== 'ANNULE' && (
                              <span className={days < 0 ? 'text-[var(--msp-red)]' : days <= 3 ? 'text-[var(--msp-amber)]' : 'text-[var(--text-secondary)]'}>
                          ...
                          {/* Aléas/incidents */}
                          {step.incidents && (
                            <div className="mt-[8px] bg-[var(--msp-amber-light)] border border-[var(--msp-amber)] rounded-[8px] p-[8px_12px]">
                              <div className="flex items-center gap-[6px] mb-[2px]">
                                ...
                                <span className="text-[11px] font-medium text-[var(--msp-amber)]">Aléas / Incidents</span>
                              </div>
                              <p className="text-[12px] text-[var(--msp-amber)]">{step.incidents}</p>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-[6px] items-center flex-shrink-0">
                        ...
                        <select
                          ...
                          disabled={isPending}
                          className="p-[4px_8px] border border-[var(--border)] rounded-[6px] text-[11px] bg-[var(--bg-card)] text-[var(--text-primary)] focus:outline-none focus:border-[var(--accent-primary)] cursor-pointer disabled:opacity-50"
                        >
                          <option value="A_VENIR">À venir</option>
                          <option value="EN_COURS">En cours</option>
                          <option value="TERMINE">Terminé</option>
                        ...
                        <button onClick={() => setStepIncident({ id: step.id, text: step.incidents || '' })}
                          className="text-[11px] text-[var(--msp-amber)] border border-[var(--msp-amber)] bg-[var(--msp-amber-light)] py-[3px] px-[8px] rounded-[6px] hover:bg-[var(--msp-amber-light)] transition-colors">
                          Aléas
                        </button>
          ...
          {stepIncident && (
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-[24px]">
              <div className="bg-[var(--bg-card)] rounded-[12px] border border-[var(--border)] w-full max-w-[480px] p-[24px]">
                <h3 className="text-[14px] font-medium text-[var(--text-primary)] mb-[12px]">Aléas, incidents ou changements</h3>
                <p className="text-[12px] text-[var(--text-secondary)] mb-[14px]">Notez tout changement survenu sur cette étape.</p>
                <textarea
          ...
          {filteredNotes.length === 0 ? (
            ...
            <div className="flex flex-col gap-[10px]">
              {filteredNotes.map(note => (
                <div key={note.id} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[10px] p-[16px_20px]">
                  <div className="flex justify-between items-start gap-[12px]">
                    <div className="flex-1">
                      {note.title && <h4 className="text-[13px] font-medium text-[var(--text-primary)] mb-[4px]">{note.title}</h4>}
                      <div className="flex flex-wrap gap-[12px] text-[12px] text-[var(--text-secondary)] mb-[8px]">
                        <span>{formatDate(note.date)}</span>
                        {note.participants && <span>Participants : {note.participants}</span>}
                      </div>
                      <p className="text-[13px] text-[var(--text-primary)] leading-[1.6] whitespace-pre-wrap">{note.notes}</p>
                    </div>
                    <button onClick={() => handleNoteDelete(note.id)} disabled={isPending}
                      className="text-[12px] text-[var(--msp-red)] hover:underline flex-shrink-0 disabled:opacity-50">
                      Supprimer
                    </button>
                  </div>
                </div>
      ...
      {tab === 'Contentieux' && (
        <div>
          <div className="flex justify-between items-center mb-[20px]">
            <p className="text-[13px] text-[var(--text-secondary)]">Registre des litiges et contentieux liés à ce marché</p>
            ...
            <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-[10px] p-[20px] mb-[20px]">
              <form onSubmit={handleContSubmit}>
                ...
                <div className="flex gap-[10px] justify-end">
                  ...
                  <button type="submit" disabled={isPending}
                    className="bg-[var(--msp-red)] text-[var(--bg-card)] py-[8px] px-[14px] rounded-[8px] text-[12px] font-medium hover:bg-[var(--msp-red)] disabled:opacity-60">
                    {isPending ? 'Enregistrement…' : 'Enregistrer'}
                  </button>
                </div>
              </form>
          ...
          {market.contentieux.length === 0 ? (
            ...
            <div className="flex flex-col gap-[10px]">
              {market.contentieux.map(c => (
                <div key={c.id} className={`bg-[var(--bg-card)] border rounded-[10px] p-[16px_20px] ${c.status === 'EN_COURS' ? 'border-[var(--msp-red)]' : 'border-[var(--border)]'}`}>
                  <div className="flex flex-col sm:flex-row justify-between items-start gap-[12px]">
                    <div className="flex-1">
                      <div className="flex items-center gap-[8px] flex-wrap mb-[4px]">
                        <h4 className="text-[13px] font-medium text-[var(--text-primary)]">{c.subject}</h4>
                        <span className={`inline-block border rounded-[6px] py-[1px] px-[6px] text-[11px] ${CONT_CFG[c.status] || CONT_CFG.EN_COURS}`}>
                          {c.status === 'EN_COURS' ? 'En cours' : c.status === 'RESOLU' ? 'Résolu' : 'Abandonné'}
                        </span>
                      </div>
                      <div className="text-[12px] text-[var(--text-secondary)] mb-[6px]">Ouvert le {formatDate(c.openDate)}</div>
                      {c.description && <p className="text-[13px] text-[var(--text-secondary)] leading-[1.5]">{c.description}</p>}
                      {c.resolution && (
                        <div className="mt-[8px] bg-[var(--msp-green-light)] border border-[var(--msp-green)] rounded-[8px] p-[8px_12px]">
                          <span className="text-[11px] font-medium text-[var(--msp-green)]">Résolution : </span>
                          <span className="text-[12px] text-[var(--msp-green)]">{c.resolution}</span>
                        </div>
                      )}
                    </div>
                      ...
                      {c.status === 'EN_COURS' && (
                        <>
                          <button
                            ...
                            disabled={isPending}
                            className="text-[11px] text-[var(--msp-green)] border border-[var(--msp-green)] bg-[var(--msp-green-light)] py-[3px] px-[8px] rounded-[6px] hover:bg-[var(--msp-green-light)] disabled:opacity-50">
                            Résoudre
                          </button>
                          <button onClick={() => handleContStatus(c.id, 'ABANDONNE')} disabled={isPending}
                            className="text-[11px] text-[var(--text-secondary)] border border-[var(--border)] py-[3px] px-[8px] rounded-[6px] hover:bg-[var(--bg-surface)] disabled:opacity-50">
                            Abandonner
                          </button>
                        </>
                      )}
